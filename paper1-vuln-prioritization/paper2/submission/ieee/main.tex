% main.tex — CalibScore / Failure-Aware Gate
...
% Author: Harshavardhan Malla, Vensoft Inc.
...
A companion contribution by the same author~\cite{Malla2026VulnPrio} constructed a reproducible, reproducibility-audited
...
The present paper asks the natural follow-on question: can we calibrate
...
empirically calibrated, a gap that motivates the calibration question
addressed here and by the companion benchmark of~\cite{Malla2026VulnPrio}.
...
audit integrity, and the freeze invariant of~\cite{Malla2026VulnPrio} on every batch?
...
to the companion placeholder of~\cite{Malla2026VulnPrio}.
...
companion placeholder vector of~\cite{Malla2026VulnPrio}, reported as a descriptive summary.  No
...
rank perturbation relative to the companion placeholder of~\cite{Malla2026VulnPrio} is small (median
...
The specific implication for the companion benchmark~\cite{Malla2026VulnPrio} is that its
...
establishes provenance for the design choices in the companion placeholder weights of~\cite{Malla2026VulnPrio} by demonstrating that the placeholder is not an outlier relative
...
sensitivity sweep code alongside this paper for replication.  The companion benchmark~\cite{Malla2026VulnPrio} remains valid and unchanged; its placeholder weights
