<!--
The CalibScore study, Step 3.7: Full multi-t0 feasibility run with NVD_API_KEY and gate.
...
The VulnPrio study (results/primary_full_v1/) untouched and freeze-verified.
-->

# the CalibScore study, Step 3.7: Full Multi-t0 Feasibility Run (NVD key), Result
...
| the VulnPrio study freeze status | **OK** (verified before and after; probe is read-only w.r.t. The VulnPrio study) |
...
CONDITIONAL; <20 → PIVOT to a robustness/sensitivity-only the CalibScore study.

## Invariants honored
The VulnPrio study frozen outputs untouched (freeze OK before/after). PoC/ExploitDB license-gated
