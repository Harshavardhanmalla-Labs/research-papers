<!--
The CalibScore study, Step 3.12: Fix F3, Pre-register Metric → Claim Binding.
...
no code written; no experiments run; the VulnPrio study frozen outputs untouched.
-->

# the CalibScore study, Step 3.12: Fix F3, Metric → Claim Binding (Pre-registered)
...
| Metric | Source | Notes |
|---|---|---|
| `compute_ehd` / `compute_pair_ehd` | `eehda.py:58` / `:29` | Exposure-Host-Day harm; **the primary operational metric for the CalibScore study** |
...
- `make verify-primary-freeze PYTHON=.venv/bin/python` → content-addressed verification of the VulnPrio study's `results/primary_full_v1/` (binary OK/FAIL).
...
| Allowed interpretation | "the VulnPrio study's frozen artefact was not perturbed by the CalibScore study work" |
...
| A2 | strategy | 13 strategies (the VulnPrio study's full list) | CLM-B1, CLM-B2, CLM-B3 | EHD; eehda_relative; fraction_of_oracle | primary for CLM-B* | **minimal factorial, REQUIRED** |
...
| `poam_review_trigger_compliance` | Compliance scaffolding from the VulnPrio study; not used as Paper-2 primary or secondary. |
...
The following claims are **forbidden** in the CalibScore study regardless of measurement
...
| *"Compliance achievement."* | Out of scope (compliance was the killed the CalibScore study topic in Step 2.5) |
...
- **F3: COMPLETE.** Metric → claim binding table locked for classes A-G; 13
 ...
 No new metrics introduced; no new axes introduced; the VulnPrio study frozen outputs
 untouched.
- **Step 4 still NOT allowed.** F4-F9 owed; F1-mandated framing changes owed.

## Invariants honored
The VulnPrio study frozen outputs untouched. No code written; no experiments run; no paper
