<!--
The CalibScore study, Step 3.18: Fix F9, Venue Plan and "Change Our Minds" Clause.
...
blockers. No code; no experiments; no F2..F8 changes; the VulnPrio study frozen outputs
untouched.
-->
...
KILL / PIVOT / KEEP the CalibScore study, enumerates the entry criteria Step 4 must satisfy,
...
5. **Cite P1-P15** from Step-3.10's table with **visible differentiation** in
 ...
 sentence stating what the CalibScore study does *differently* (not just *additionally*).
...
### 5.1 KILL conditions (any one → abandon the CalibScore study)

| ID | Trigger | Action |
|---|---|---|
| **K9-KILL-A.1** (venue) | F9 venue search and 2 follow-on attempts find **no plausible home** for a negative / methodology result (CSET + LASER + DTRAP all return reject decisions in the same submission cycle) | KILL the CalibScore study; do **not** rewrite for a stretch-fit venue |
| **K9-KILL-A.2** (compute) | Pilot runtime measurement gives a projected primary runtime that **exceeds F8 hard max even after the full F8-e → F8-a fallback escalation** (F8-a halves seeds; if that still doesn't fit 18 h, the design is infeasible on the available hardware) | KILL the CalibScore study unless explicit user OK to expand the compute envelope |
| **K9-KILL-A.3** (K2 + judgment) | K2 fires on **every** primary axis **and** an independent reviewer regards the negative finding as "trivial" (i.e., the failure-aware-gate methodology is rejected as "standard ML feasibility analysis") | KILL the CalibScore study; the surviving novelty has collapsed |
| **K9-KILL-A.4** (degeneracy) | Sparsity makes **every** outcome degenerate (per-cell paired-Δ identically zero on every primary cell, every sensitivity axis, every metric, beyond `kev_first` alone) | KILL the CalibScore study; nothing to report beyond the sparsity figure itself |
| **K9-KILL-A.5** (insight) | After the primary completes, the paper produces **no meaningful difference and no methodological insight** beyond Step 3.8's sparsity number itself, and that number alone is insufficient for *any* of the §4 venues | KILL the CalibScore study |
| **K9-KILL-A.6** (prior art) | A *later-discovered* prior-art source (post-F1) is found to publish the same failure-aware gate methodology under public-feed labels with a comparable sparsity figure | KILL the CalibScore study unless a sharper differentiation can be written without inventing novelty |
...
| **K9-PIVOT-B.2** | Catalog expansion turns out to be necessary to make any sensitivity claim meaningful, but catalog expansion is **deferred** by Step 3.9 / F6 | Pivot to a Paper-2.5 / Paper-3 catalog-expansion preregistration; the CalibScore study publishes as currently scoped only if the failure-aware gate alone holds up |
...
| VulnScore (Alqahtani & Almukaynizi, Springer IJIS 2025, **10.1007/s10207-025-01164-3**) | `[VERIFY]` retained | Not re-fetched this turn; used only as soft-context in Step-3.11 F2 table for composite-scoring framing. If retained in the CalibScore study's Related Work, re-verify at submission. |
...
## Invariants honored
The VulnPrio study frozen outputs untouched. No experiments; no code; no F2/F3/F4/F5/F6/F7/F8
