<!--
The CalibScore study, Step 3.8: Resumed full multi-t0 feasibility run with NVD_API_KEY.
Real public-feed measurement (NVD/EPSS/KEV); no counts fabricated; key value never
echoed or written to any artifact. The VulnPrio study (results/primary_full_v1/) untouched and
freeze-verified.
-->

# the CalibScore study, Step 3.8: Full Multi-t0 Feasibility Result (NVD key), PIVOT
...
calibrated-prioritization the CalibScore study is NOT allowed.** The recommended path is to
pivot the CalibScore study to a robustness/sensitivity-only study (real-feed features with
...
## Step 2, the VulnPrio study freeze
...
| the VulnPrio study freeze | **OK** (verified before and after) |
...
**Step 4 pre-registration for the calibrated-prioritization the CalibScore study is NOT allowed.**

## Pivot options (in priority order; ALL still require explicit user GO and a fresh Step 4)
1. **Robustness / sensitivity-only the CalibScore study** (recommended): real public-feed features
 ...
 imputation/coverage sensitivity on a real-public-fleet-shaped extension of the VulnPrio study's
...
3. **the HygieneBench benchmark alternative** entirely (e.g., the KEV-prediction or scheduler-policy
...
## Invariants honored
The VulnPrio study frozen outputs untouched (`verify-primary-freeze` OK before and after; probe
