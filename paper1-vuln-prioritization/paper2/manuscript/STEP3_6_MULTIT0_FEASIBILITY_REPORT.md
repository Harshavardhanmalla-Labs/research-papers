<!--
The CalibScore study, Step 3.6: Full-Window Acquisition Fix and Multi-t0 Feasibility Probe.
...
for the full-scale measurement (NVD_API_KEY). No counts are fabricated. The VulnPrio study
(results/primary_full_v1/) untouched and freeze-verified.
-->

# the CalibScore study, Step 3.6: Multi-t0 Feasibility & Hardened Acquisition Report
...
- `tests/test_paper2_feasibility_probe.py`: **+9 tests** (now 19/19 pass) covering
 ...
 the VulnPrio study freeze untouched.
...
- **<20 even at full scale → PIVOT** to a robustness/sensitivity-only the CalibScore study
 (Label A/B, capacity, blackout, imputation sweeps on real-feed features with
 fixed/published weights), or a the HygieneBench benchmark alternative.

## Invariants honored
The VulnPrio study frozen outputs untouched (`verify-primary-freeze` OK before and after; probe is
