<!--
The CalibScore study, Step 4: Pre-Registration Document.
...
The VulnPrio study modification. Step 5 (runner implementation) not started.
-->

# the CalibScore study, Step 4: Pre-Registration
...
| the VulnPrio study frozen outputs modified | **None** (read-only verifications only) |
...
This document is the **immutable design contract** for the rest of the CalibScore study.
...
This pre-registration commits the CalibScore study to study **when** per-feature
...
7 < 20 fires PIVOT). The CalibScore study therefore makes **no learned-calibration
...
sensitivity sweep on a real-feed extension of the VulnPrio study's frozen
...
fleet-equals-real-host claim.** the VulnPrio study's frozen primary artefact remains
...
- **Q5 (audit / reproducibility, CLM-F1 / F2):** Are outputs content-
 addressed and audit-verifiable, and is the VulnPrio study's freeze invariant
...
## 6. Framing changes from F1, committed (one line each on how the CalibScore study honors them)

| # | Step-3.10 §9 requirement | How the CalibScore study honors it |
|---|---|---|
...
| 5 | Cite P1-P15 with **visible** differentiation in Related Work | Each P1-P15 citation must be followed by exactly one sentence stating what the CalibScore study does differently (not just additionally). This is a Step-7 manuscript-CI check. |
...
- `w_uniform`,
- `w_paper1_placeholder` (= the VulnPrio study's existing `w_hand`),
- `w_epss_dominant`,
- `w_cvss_dominant`,
- `w_kev_dominant`,
- `w_context_dominant`.
...
(F2 W4); no learned weights may be introduced in the CalibScore study (F2 W5 + F5 K1
forbid `register_calibrated_weights` calls).

`w_paper1_placeholder` must always be labelled **"the VulnPrio study placeholder
...
- **Change-our-minds clause** (carried verbatim from F9):
 - **KILL (any one → abandon the CalibScore study):** K9-KILL-A.1 no venue home after
...
- No learned-model / GBT / DRL comparator anywhere in the CalibScore study (K1).
...
- the VulnPrio study frozen outputs **untouched**.
- No experiments run.
- No code written.
