<!--
The CalibScore study, Step 3.13: Fix F4, Pre-register Minimum Detectable Effect (MDE) /
...
experiments run; no calibration; the VulnPrio study frozen outputs untouched.
-->

# the CalibScore study, Step 3.13: Fix F4, MDE / Power Pre-registration
...
behaviour, (ii) the variance prior taken from the VulnPrio study's frozen 30-seed EHD
...
## 2. Variance prior, extracted from the VulnPrio study frozen artefact (read-only)
...
- `oracle - epss_only` mean Δ is **negative** (-2,576 hd; oracle is *worse*
 ...
 artefact (the VulnPrio study finding); pre-registered as fully expected for the CalibScore study.
...
*smaller than the per-seed paired-Δ standard deviation* observed in the VulnPrio study
...
| CLM-B1, `kev_first` vs `epss_only` | EHD (paired Δ) | 30 | 5,000 | n/a (std=0) | **DROPPED** | none | report "identically equal under fixtures; the CalibScore study must re-confirm under real feeds; if still degenerate, leave dropped" |
...
3. `MDE_host_days ≤ 5,000` at the chosen sparsity inflation factor (default ×1.5 for the CalibScore study; if the run encounters per-cell positive count < 1, the inflation factor is escalated to ×2.0 *prospectively*, not after seeing the test outcome).
...
### 6.1 Family definitions (the only Holm families allowed in the CalibScore study; F6 may shrink but not grow)
...
## Invariants honored
The VulnPrio study frozen outputs untouched (variance prior is a read-only extraction).
