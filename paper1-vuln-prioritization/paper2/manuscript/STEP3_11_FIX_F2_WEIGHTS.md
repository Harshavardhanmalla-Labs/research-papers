<!--
The CalibScore study, Step 3.11: Fix F2, Lock the Fixed-Weight Source.
Documents the locked, pre-registration-equivalent fixed-weight set for the CalibScore study's
...
no calibration; the VulnPrio study frozen outputs untouched.
-->

# the CalibScore study, Step 3.11: Fix F2, Locked Fixed-Weight Source

**F2 status: COMPLETE.**
**Chosen strategy: Option C, weight choice is itself a sensitivity axis.**
**Step 4 still NOT allowed** (F3-F9 owed).

The decision: the CalibScore study will treat the prioritization weight vector as a
...
## 1. Inspected: existing the VulnPrio study weight registry
...
```
- `E`, EPSS exploit-likelihood (real-feed in the CalibScore study).
- `K`, KEV-listing status as of t0 (real-feed in the CalibScore study).
- `S`, CVSS severity (real-feed in the CalibScore study; v3.1 / v4 per NVD).
...
them with regression-fit values."* the VulnPrio study's `proposed_full` strategy uses
`weights_name="w_logit_placeholder"` (`src/paper1/model/strategies.py:99`).

### 1.4 Are placeholders named as placeholders? **Yes.**
Both module docstring and variable names ("placeholder") are explicit. the VulnPrio study's
manuscript also tags `proposed_full` results as "placeholder-weight" outcomes
(the VulnPrio study closeout package). No silent re-labelling has occurred.
...
`register_calibrated_weights`, the VulnPrio study documented this gap; the CalibScore study Steps 3.6-3.8
...
- `ablate_weight` zeroes a feature and renormalises the remaining six (used by
 the VulnPrio study's `proposed_no_criticality` and `proposed_no_exposure` strategies).
...
- **Option A, the VulnPrio study placeholder weights only.** Honest, reproducible, but
...
| **`w_paper1_placeholder`** *(already registered as `w_hand` / `w_logit_placeholder`)* | 0.20 | 0.20 | 0.10 | 0.20 | 0.15 | 0.10 | 0.05 | 1.00 | The *exact* pre-existing the VulnPrio study placeholder weights. Carried forward unchanged so the CalibScore study's `proposed_full` cell is comparable to the VulnPrio study's frozen artefact. **Must always be labelled "the VulnPrio study placeholder weights"**, never "optimized" / "calibrated" / "learned" / "validated" / "literature-derived". | the VulnPrio study frozen artefact `results/primary_full_v1/` (self-reference, documented placeholder) |
...
- **W5.** **No learned weights in the CalibScore study.** Per Step 3.8, calibration is
...
- **W7.** **The `proposed_full` strategy's labelling rule is unchanged from
 the VulnPrio study**: it remains a *placeholder-weight* result. The CalibScore study may run
...
5. No changes to `register_calibrated_weights` (kept inert in the CalibScore study).
6. **the VulnPrio study's frozen outputs and existing built-ins are never modified.**
...
The VulnPrio study frozen outputs untouched. No experiments run; no code written; no paper
