# the CalibScore study, Original Contribution Statement

This file states what is original in the CalibScore study *as a methodology artefact*,
...
The CalibScore study explicitly positions itself against the public prior-work entries
...
differentiation. Examples of prior work and how the CalibScore study differs:

- Prior work that fits per-feature weights on a single time origin and a
 partial vulnerability label: the CalibScore study evaluates the *feasibility* of that
 ...
 dataset.
- Prior work that proposes new ranking signals: the CalibScore study takes the existing
...
- Prior work that reports positive prioritization results: the CalibScore study reports a
...
- The VulnPrio study frozen comparator scoring formula (it is reused, not
 re-derived, and the VulnPrio study frozen outputs were not modified).
...
the CalibScore study's originality argument does not rest on a comparator-beating
