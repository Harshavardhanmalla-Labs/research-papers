# the capacity-decay study, Capacity-Indexed Decay of Exploit-Likelihood Vulnerability Prioritization

A pre-registered two-dimensional sweep over remediation capacity $K$ and
CVE arrival rate $\lambda$ extending the temporal-stability study's multi-window simulation.
...
Re-uses the HygienePrio scorer's `hygieneprio` package and the temporal-stability study's `paper5.window_sim`
