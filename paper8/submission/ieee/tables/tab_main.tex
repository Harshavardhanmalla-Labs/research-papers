\begin{table*}[!t]
 \caption{Mean P@50 per (capacity $K$, window) for all five calibration strategies. EWMA and trail3 are the multi-history smoothers introduced in this paper; offline is the online-calibration study ceiling. Cells marked ``, '' have an offline$-$fixed gap $\leq 0.01$ (including W1, where all strategies coincide), so the recovery ratio $\rho$ is not reported.}
 \label{tab:main}
 \centering
 \small
