...
def main():
    ...
    for pid, root, pdf in rows:
        ...
        try:
            ...
            # Strip every build artifact from the temp tree. Critical: a stale
            # main.pdf (the non-anonymized author copy) would otherwise survive a
            # failed compile and get copied out as anonymous.pdf -> a silent name
            # leak. Remove all PDFs and aux files so only a fresh compile can win.
            for dirpath, _, fnames in os.walk(tmp):
                for fn in fnames:
                    if fn.endswith((".pdf", ".aux", ".log", ".out", ".blg",
                                    ".synctex.gz", ".fls", ".fdb_latexmk", ".toc")):
            ...
            if p.returncode == 0 and os.path.isfile(outpdf):
