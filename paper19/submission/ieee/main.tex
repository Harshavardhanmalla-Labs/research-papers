...
\begin{abstract}
A configuration management database (CMDB) is the asset inventory on which security and cost management
...
cost. Prior asset-visibility and record-linkage work treats inventory accuracy as a single number to
maximize; our contribution is to separate and quantify the two opposing costs from a single set of
results, and to show that they have opposite remedies, so reducing one tends to raise the other and
the operating point that minimizes their sum sits at an interior optimum, not at either extreme.
The operational implication is concrete: asset-inventory hygiene is a tuning problem, not a chase to
zero, and the right operating point is set by how an organization prices a security blind spot against
a wasted-spend record. We quantify both errors with a
...
This paper quantifies the benefit and its bounds. The novel contribution is to make the two-sided cost
of CMDB error explicit and to quantify both sides from a single set of pre-registered results: the
ghost rate, a security cost, and the phantom rate, a financial and noise cost, measured together so
that the tradeoff between them is visible rather than hidden inside an aggregate accuracy figure. What
prior asset-visibility mandates~\cite{nist80053,cisabod2301} and record-linkage
theory~\cite{fellegi1969linkage,christen2012matching} did not show is that these two costs move in
opposite directions under operational tuning, so that there is an interior operating point and a hard
matching-precision floor rather than a monotone accuracy gain. The operational implication is direct:
inventory hygiene should be tuned to the interior optimum set by the organization's security-to-cost
balance, not driven toward zero error on either axis. We do not claim to measure any particular
...
vulnerability-prioritization study for critical-infrastructure and public-sector endpoint
fleets~\cite{paper11capg} and the asset-criticality-driven patch-triage
study~\cite{paper14patch}, depends on the inventory being
right in the first place, since an asset absent from the inventory is absent from every
prioritization that reads it, and asset criticality cannot be assigned to an asset the inventory
does not record.
...
\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig1_regime.pdf}
...
\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig2_tradeoff.pdf}
...
\bibitem{paper11capg}
H.~Malla, ``Context-aware vulnerability prioritization for critical-infrastructure and public-sector endpoint fleets,'' \url{https://github.com/Harshavardhanmalla-Labs/research-papers/tree/main/paper11}, 2026, companion paper.

\bibitem{paper14patch}
H.~Malla, ``Patch {Tuesday} triage: asset criticality, not exploit likelihood,'' \url{https://github.com/Harshavardhanmalla-Labs/research-papers/tree/main/paper14}, 2026, companion paper.
