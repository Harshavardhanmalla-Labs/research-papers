<!--
The CalibScore study, Step 3: Data Acquisition Runbook for the public-feed feasibility probe.
Run in a NETWORK-ENABLED environment (or with cached snapshots). Uses public
feeds only (NVD/EPSS/KEV); PoC/ExploitDB stays OFF (license-gated). The VulnPrio study frozen
outputs are never touched. No counts may be fabricated; if blocked, report the
blocker honestly.
-->

# the CalibScore study, Data Acquisition Runbook (Feasibility Probe)

Goal: obtain real (or cached) public-feed data so the probe can measure the
distinct-positive-CVE count that gates the CalibScore study.
