<!--
The CalibScore study, Step 3.9: Research Validation Report for Pivot Option A.
Robustness/sensitivity-only the CalibScore study using real public-feed features (NVD/EPSS/KEV) and
FIXED weights, on a real-feed extension of the VulnPrio study's frozen synthetic-fleet benchmark.
...
# the CalibScore study, Step 3.9: Pivot Decision and Research Validation Report
...
- **The calibration paper is infeasible under the current public-feed + 31-product
 ...
 not attempted.
- **Recommended pivot: Option A, robustness/sensitivity-only the CalibScore study** using real
 public-feed features and **fixed weights** (no calibration claim, no superiority
 claim) on a real-feed extension of the VulnPrio study's frozen synthetic-fleet benchmark.
...
## 4. How this differs from the VulnPrio study
| Dimension | the VulnPrio study (frozen) | the CalibScore study pivot |
|---|---|---|
| Inputs | Toy synthetic fixtures | **Real NVD / EPSS / KEV public feeds** + same synthetic fleet generator |
| Weights | Placeholder | **Fixed** (placeholder from the VulnPrio study *or* literature-prior; published-weight source must be `[VERIFY]`-checked before locking) |
...
| Runs | One primary frozen run (30 seeds × 13 strategies × 11 metrics) | **Sensitivity sweeps** across capacity / blackout / approver / feature-ablation / label-source axes, on a **read-only extension** of the VulnPrio study's frozen benchmark |
...
| Frozen outputs touched | All produced by the VulnPrio study | **None**, the VulnPrio study's `results/primary_full_v1/` stays content-addressed-frozen |

The VulnPrio study ↔ the CalibScore study boundary is therefore: same code skeleton + same fleet
generator + new public-feed feature attach + new sensitivity sweeps, with the VulnPrio study's
...
- **C1. Empirical characterisation of public-feed label sparsity** for a 31-product
 ...
 Step-3.8 measured).
- **C2. Real-feed extension of the VulnPrio study's frozen benchmark**, same scheduler /
...
- **C5. Reproducible public-feed acquisition + freeze pipeline**: chunked / resumable
 ...
 dedup, content-addressed freeze (inherited from the VulnPrio study).
...
| Strategy | random, cvss_only, epss_only, kev_first, cvss_x_epss, cvss_plus_epss_plus_kev, cve_max, cve_mean, cve_sum, proposed_full *(fixed weights)*, proposed_no_criticality, proposed_no_exposure, oracle (13, same as the VulnPrio study) |
...
The Step-1 instruction to *not* run full the CalibScore study experiments is honored here.

## 7. Metrics (kept honest about what each can and cannot say with 7 positives)
| Metric (from the VulnPrio study's 11) | Use under sparse labels | Caveat |
|---|---|---|
...
- **Paired seed comparisons** within each axis cell (same seed across strategies),
 reusing the VulnPrio study's seed scheme for compatibility.
...
- **MDE / power analysis pre-registered**: for each axis × metric, compute the
 minimum detectable effect at our seed count (≤30 per cell, matching the VulnPrio study) and
...
| the VulnPrio study frozen artefact reusable as control | **yes**, content-addressed freeze verified before/after every run |
...
| Catalog expansion (deferred) | future work; not pursued in the CalibScore study per Step 3.9 |
...
9. **Frozen-weights provenance.** If we use the VulnPrio study's *placeholder* weights, the
...
Pivot away from this Option-A paper (and consider the HygieneBench benchmark) if **any** of the
...
- **K6.** the VulnPrio study's freeze verification fails at any point in the CalibScore study work.
...
**F2. Lock fixed-weight source (blocking).** Either (i) reuse the VulnPrio study's documented
...
**F7. The VulnPrio study freeze invariant** must be re-asserted in the pre-registration: every
...
- the VulnPrio study frozen outputs untouched (`results/primary_full_v1/`); `verify-primary-freeze`
