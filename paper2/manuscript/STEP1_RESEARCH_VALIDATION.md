...
moderate "analysis/results companion" to the VulnPrio study**, with real incrementality risk
...
## 2. Research problem
The VulnPrio study left a precise open question: under a capacity-constrained,
...
blackout/approver policy, and EPSS version?** the VulnPrio study used placeholder weights, a
...
## 5. How it differs from the VulnPrio study

| | the VulnPrio study | the CalibScore study (this) |
|---|---|---|
...
## 6. How it reuses the VulnPrio study without duplicating it
...
re-describe the framework in detail (cite the VulnPrio study). **Do not** touch
`results/primary_full_v1/`; the CalibScore study writes to a **new results namespace** (e.g.,
`results/paper2_*`) with its own freeze manifests. Duplication risk is mitigated by
making the CalibScore study mostly *experiment configs + analysis + writing* with minimal new
...
- **Calibrated logistic / ridge weights** (Phase 6, built). ⚠️ **Critical caveat:**
 calibration is meaningless on the 5-CVE toy fixtures. The CalibScore study **must** use a
 ...
 temporal train/gap/test. Biggest design requirement; also *reduces* the VulnPrio study's
 "toy" limitation.
...
## 10. Required metrics (reuse the VulnPrio study)
...
CIs**; **paired Cohen's d**; **minimum detectable effect** (report it, the VulnPrio study
...
reproducibility methodology. All `[VERIFY]`; reuse the VulnPrio study's confirmed Tier-1 set;
**no fabrication**.
...
- If it does not → report plainly (as the VulnPrio study did). Do not bury it.
...
employer data. Keep the VulnPrio study's data warning.
...
1. Introduction (the open question from the VulnPrio study; pre-registered) · 2. Background
...
5. Benchmark Recap (cite the VulnPrio study; reused vs new) · 6. Data & Labels (public-feed
...
and a clean companion to the VulnPrio study, but moderate-novelty and likely neutral-result;
...
**Next step options:** (a) proceed to the CalibScore study Step 2 (prior-art falsification for
this topic) and draft the pre-registration plan; or (b) weigh a more distinct
The CalibScore study/3 topic first. No drafting has occurred.
