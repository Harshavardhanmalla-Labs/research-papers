<!--
The CalibScore study, Step 3.16: Fix F7, Paper-1 Freeze Invariant Contract.
Locks the wrap pattern, audit-record schema, CI/manuscript gate, and weight-
registry test that every Paper-2 run must obey so the VulnPrio study's frozen artefact
(results/primary_full_v1/) is never corrupted, mutated, re-frozen, or
reinterpreted. No code, no experiments, no F2..F6 changes; the VulnPrio study frozen
outputs untouched.
-->
...
### 4.1 Allowed (the CalibScore study may create / write / read)
...
as the VulnPrio study (write to `*.tmp` then rename).
...
exit codes, status, and a one-line statement of either "the VulnPrio study freeze
invariant held" or "the VulnPrio study freeze invariant violated; batch invalid; no
...
# (v) Paper-1 built-ins cannot be overwritten by the CalibScore study.
import pytest
...
# (vi) the CalibScore study may register new F2 vectors at non-built-in names without
...
6. The Paper-1 weight-registry lock test (§8) lands in `tests/` and runs in
 the same `pytest` invocation as the CalibScore study's own tests.
...
## Invariants honored
The VulnPrio study frozen outputs untouched (this step only **read** the manifest path,
