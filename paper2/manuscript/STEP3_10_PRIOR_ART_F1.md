<!--
The CalibScore study, Step 3.10: Fix F1, Prior-Art Verification (executed).
Targeted web/library prior-art search for the Step-3.9 pivot
("robustness/sensitivity-only the CalibScore study"). Verified sources only;
unverified claims carry [VERIFY]. No fabricated citations.
The VulnPrio study (results/primary_full_v1/) untouched; no experiments run.
-->

# the CalibScore study, Step 3.10: Prior-Art Verification (Fix F1)
...
- No employer/production data. No the VulnPrio study freeze touched. No experiments run.
...
- **C2** (real-feed extension of the VulnPrio study benchmark), **engineering, not novelty**.
...
- **NOT KILL.** The negative feasibility finding + the failure-aware-gate methodology + the reproducible probe artefact are not in the published literature in this combination (F1 search). The VulnPrio study → the CalibScore study reproducibility story is also coherent.
...
The VulnPrio study frozen outputs untouched. No experiments run. No paper drafted. No
