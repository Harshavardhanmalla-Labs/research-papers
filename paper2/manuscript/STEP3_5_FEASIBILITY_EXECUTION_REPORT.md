<!--
The CalibScore study, Step 3.5: Public-feed feasibility probe EXECUTION report.
Real public data (NVD/EPSS/KEV) was fetched and the probe was run. No counts are
fabricated. The VulnPrio study (results/primary_full_v1/) untouched and freeze-verified.
-->

# the CalibScore study, Step 3.5: Feasibility Probe Execution Report
...
3. Re-run and re-apply the gate:
 ...
 - <20 even after fixes → **PIVOT** to a robustness/sensitivity-only the CalibScore study
 (Label A/B, capacity, blackout, imputation sweeps on real-feed features with
 fixed/published weights), or the HygieneBench benchmark alternative.

## the VulnPrio study integrity
**Untouched / freeze verified.** `verify-primary-freeze` → OK; 0 files modified
under `results/primary_full_v1/`. Probe is read-only w.r.t. The VulnPrio study.
