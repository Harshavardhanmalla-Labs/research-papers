...
\begin{abstract}
Expressing NIST 800-53 controls as executable policy-as-code lets a federal system collect control
evidence continuously, but the field has reported only that drift is caught faster, never how much
compliance exposure the executable form actually removes or what caps it. We give the first
quantified answer: detection latency and exposure reduction are distinct payoffs, the former
collapses by two orders of magnitude while the latter is hard-capped by the share of controls that
can be expressed as code, and we measure that cap. The operational payoff is a sizing rule, exposure
reduction tracks the automatable-exposure share rather than detection latency, so a program can be
budgeted against the catalog instead of against collection speed. Federal systems must show that
their security controls remain effective over time, not only at an
...
controls that can be expressed this way. A companion study takes policy-as-code further still, using
it for preventive enforcement that blocks recurring control violations before they occur rather than
detecting them after~\cite{paper13policy}; here we hold the detection framing fixed and measure the
exposure it removes.
...
This paper quantifies both the benefit and its bounds. Prior policy-as-code work established that a
control expressed as an executable rule can be evaluated on a short cadence and that drift is then
caught faster; what it never measured is how much compliance exposure that executable form actually
removes, or the hard limit on that removal. Our central quantified result supplies both: continuous
evaluation of policy-as-code controls cuts detection latency by a factor of 272 yet removes only
0.567 of annual exposure against a measured ceiling of 0.643, because the share of controls that can
be expressed as code caps the exposure that any collection speed can remove. This separation of a
latency payoff from an exposure payoff, and the measurement of the ceiling on the latter, is what
prior compliance-as-code accounts did not show, and it converts the case for automation from a
latency story into a budgetable sizing rule. We do not claim to measure any particular
...
\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig1_ceiling.pdf}
...
floor formed by controls a machine cannot check.\\[2pt]\textit{Alt text:} A two-bar chart on a y-axis labeled fraction of annual exposure removed, from 0 to 1. The left bar, realized continuous reduction, reaches about 0.57; the right bar, automatable-share ceiling, reaches about 0.64, both with small error bars. A shaded band above 0.64 is labeled non-automatable floor.}\label{fig:ceiling}\end{figure}

\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig2_mttd_cadence.pdf}
...
tightens from annual to quarterly.\\[2pt]\textit{Alt text:} Two side-by-side bar charts. Left, on a logarithmic y-axis of mean time to detect in days, an annual-assessment bar at 272 days towers over a continuous-as-code bar at 1 day. Right, on a linear y-axis of control-days saved per fleet over two years, the over-annual bar reaches about 33,000 and the over-quarterly bar about 15,600, both with error bars.}\label{fig:mttd}\end{figure}
