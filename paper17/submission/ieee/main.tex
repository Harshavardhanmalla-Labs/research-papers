...
inspected. A four-ring rollout reduces the expected blast radius of a faulty change, the fraction of
...
before inspecting any evaluation data, and report what the model says. The contribution is fourfold.
...
self-correcting enforcement; ring rollout is the safety scaffold that keeps such continuous
...
enforced and how its effectiveness is to be tracked over time, and binding operational
...
\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig1_config.pdf}
...
0.761, 0.1912, and 0.0353 at the labelled convergence cost.}\label{fig:config}\end{figure}

\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig2_detect.pdf}
\caption{The four-ring expected blast radius rises from 0.0156 toward the big-bang value as the
per-stage detection probability falls from 0.95 to 0.2.}\label{fig:detect}\end{figure}
