# the HygieneBench benchmark, Step 2: Dataset Feasibility Report
...
Evaluate whether the HygieneBench benchmark can be built using fully synthetic telemetry, confirm or rule out the availability of publicly citable structural priors for each schema component, establish the synthetic generation strategy, and explicitly reconfirm distinctness from the VulnPrio study and the CalibScore study.
...
## 5. Distinctness Reconfirmation, the VulnPrio study and the CalibScore study

### 5.1 Distinctness from the VulnPrio study

| Dimension | the VulnPrio study | the HygieneBench benchmark |
|---|---|---|
...
| Overlap | None on unit of analysis; both share reproducibility ethos | the HygieneBench benchmark adds identity / AD / telemetry-freshness dimensions not in the VulnPrio study |
...
### 5.2 Distinctness from the CalibScore study

| Dimension | the CalibScore study | the HygieneBench benchmark |
|---|---|---|
...
| Synthetic data | Vulnerability × host synthetic data (the VulnPrio study lineage) | New identity-endpoint-patch-vulnerability telemetry schema (no overlap) |
...
**Verdict: DISTINCT.** the CalibScore study focuses on calibration quality for an existing prioritization task. The HygieneBench benchmark focuses on detecting hygiene anomalies in a new multi-domain schema. The failure-aware reporting principle is shared but applied to a different task and data structure.

**Protective clause:** the HygieneBench benchmark must not use the same synthetic dataset as the VulnPrio study or the CalibScore study. The schema defined in `SCHEMA_v0_1.md` is structurally new.
...
| Distinctness from the VulnPrio study/2 | **Confirmed.** No unit-of-analysis, schema, or primary-contribution overlap. |
