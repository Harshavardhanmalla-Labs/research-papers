...
pre-registered failure protocol. Our central finding is that \textbf{83.8\% of
...
\begin{itemize}[leftmargin=*,noitemsep,topsep=2pt]
  ...
  \item[\textbf{C5}] Explicit negative-result reporting for 83.8\% of
...
\begin{tikzpicture}[node distance=0.55cm and 0.4cm]
  ...
  \node[graybox, minimum width=6.8cm, text width=6.6cm, below=0.45cm of metrics] (results)
    {810 Primary Runs $\rightarrow$ 197/235 Configs Flagged (83.8\% \fp)\\$\rightarrow$ CSV + JSON Manifests};
...
\textbf{197 of 235 (condition, task, method) configurations are failure-flagged
(83.8\%).} The rule baseline (M1) achieves the highest mean AP (0.916). M5 (OCSVM)
is the strongest ML method (AP~=~0.913, failure rate~68.6\%). M8 (graph) is
...
M5 OCSVM            & 0.913 & \textbf{0.447} & 68.6\%~\fp \\
M8 Graph-IF$^\dagger$ & 0.801 & 0.348 & \textbf{100\%}~\fp \\
M4 LOF              & 0.800 & 0.445 & 71.4\%~\fp \\
M6 Linear-AE        & 0.798 & 0.407 & 85.7\%~\fp \\
M3 Isolation Forest & 0.781 & 0.428 & 85.7\%~\fp \\
M7 Temporal z-score & 0.774 & 0.392 & 85.7\%~\fp \\
M2 Hybrid scorer    & 0.709 & 0.417 & 94.3\%~\fp \\
\bottomrule
\end{tabular}
\end{table}
...
from 0.782 (C-BASE) to 0.613 (C-STALE, $\Delta$~=~$-$0.169). T3 combines
...
C-UNSUP & 0.844 & $+$0.072 & Instance-level sampling \\
C-BASE  & 0.772 & ---       & Reference \\
C-STALE & 0.741 & $-$0.031 & T3 $-$0.169 \\
\bottomrule
\end{tabular}
\end{table}
...
M8 (graph-augmented IF) is failure-flagged on all 25 applicable configurations
...
lowest failure rate (68.6\%) and is not flagged on T2 or T5, the two tasks where ML
adds value (it does fail Eq.~\eqref{eq:failure} on T2 in occasional individual
seeds, but never in $\geq$2/3 seeds). M2 (hybrid scorer) has the
highest failure rate among non-graph methods (94.3\%), indicating that its
...
\begin{figure}[t]
  ...
  \caption{Failure-flag fraction per (method, task) cell across all conditions and seeds. Red = high failure rate ($\geq$80\%); blue = low failure rate. M8 (graph-IF) is uniformly red (100\% failure rate). M5 (OCSVM) achieves the lowest failure rate (68.6\%).}
  \label{fig:failure_heatmap}
\end{figure}

T7 (escalation drift) is rule-dominated despite extreme rarity: with only 2
anomalous entities per dataset instance (imbalance $\approx$1:500), the rule-based
methods M1 and M7 achieve AP~=~1.000, while several ML methods rank the two rare
anomalies poorly---M2 has mean T7 AP~=~0.319, M5 0.786, M4 0.830, and M3/M8 0.861
(task mean 0.822). No ML method closes the gap to the rule baseline.

% =====================================================================
\section{Discussion and Limitations}
\label{sec:discussion}

\textbf{Interpretation.} The 83.8\% failure rate is not a finding against ML in
...
$-$0.169 AP drop on T3 under C-STALE is a conservative lower bound on what SOCs risk
...
as well as unsupervised ML on \textbf{83.8\%} of (condition, task, method)
...
M8 (graph-IF) is failure-flagged on all 25 applicable configurations. Telemetry
staleness consistently degrades performance; T3 (endpoint--identity correlation) is
the most sensitive task ($-$0.169 AP under C-STALE), quantifying the detection cost
