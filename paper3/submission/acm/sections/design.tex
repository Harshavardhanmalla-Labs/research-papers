...
label table (Table~\ref{tab:schema}). The generator
(\texttt{Synthetic\allowbreak Hygiene\allowbreak Generator}) is implemented in
Python using NumPy and Pandas, with all randomness gated through
\texttt{numpy.\allowbreak random.\allowbreak default\_rng(seed)}. Entity identifiers are
deterministic UUIDs derived from
\texttt{hashlib.md5(\allowbreak seed\_tag + index)}, ensuring reproducibility
across platforms.

\input{tables/tab_schema}

\paragraph{Structural priors.}
Patch-lag distributions use DBIR 2026 aggregate
statistics~\cite{verizon_dbir_2026} (43-day mean for critical
