...
\begin{figure}[t]
 ...
 \caption{Mean Precision@$K$ ($K \in \{50,100,250\}$) for all methods across 25 evaluation seeds (synthetic evaluation, mean $\pm$ 95\% BCa CI). HygienePrio consistently outperforms all baselines at every $K$.\\[2pt]\textit{Alt text:} A grouped chart with the vertical axis showing mean Precision@$K$ and grouped along $K = 50, 100, 250$, comparing the nine methods. Each point carries a 95\% confidence-interval error bar. The HygienePrio variants sit highest, while EPSS-only, CVSS-only, and Random sit progressively lower at every value of $K$.}
