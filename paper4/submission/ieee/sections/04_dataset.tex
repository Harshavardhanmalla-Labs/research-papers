...
All experiments use the Extended Enterprise Hygiene Dataset Artifact (EEHDA) synthetic fleet generator --- the same generator used in VulnPrio and HygieneBench, extended to output both the vulnerability-host pair tables required by VulnPrio and the hygiene-state tables required by HygieneBench. This shared generator enables the cross-paper synthesis in \S8 and ensures that HygienePrio's hygiene inputs are drawn from the same distributional assumptions as the VulnPrio baseline.
...
Thirty seeds are generated deterministically. Five seeds are designated as the calibration set (held out from primary evaluation, used only for weight grid search; see \S5.4). The remaining 25 seeds constitute the evaluation set. The calibration/evaluation split is fixed in the pre-registration protocol archived with the reproducibility artifact and is not adjusted based on results.
