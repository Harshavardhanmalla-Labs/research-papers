...
The empirical gap between the benchmark ($+30.9$ pp) and real-distribution
...
observations bounded by the inequalities (\ref{eq:cmi_bound}):
...
\textit{(ii) The benchmark gap exceeds any plausible real-data MI
...
$I(Y_{\text{bench}}; X_H \mid X_E)$ structurally large by design.
The benchmark ceiling is therefore an upper bound on what hygiene
augmentation could achieve \emph{under the benchmark labelling rule},
...
\subsection{Conclusion of the Bound Argument}

A precise information-theoretic ceiling computation requires real-fleet
...
limit.
\item The benchmark Precision@K gaps observed for the HygienePrio scorer exceed what
real-data information budgets plausibly support, consistent with the
distribution-dependent component of the headline identified in
...
\subsection{Caveats on the Estimates}
...
constructed noisy correlate of $Y$ rather than a real-fleet HRS measurement.
