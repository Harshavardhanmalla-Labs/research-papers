...
Papers~5-8 of the VulnPrio sequence, including the capacity-decay study~\cite{paper6capacity},
...
under a counterfactual trajectory. The threat is a closed-loop instance
of the more general problem of evaluating a model under a non-stationary,
self-induced distribution shift~\cite{gama2014survey}: here the scorer's
own selection policy is what moves the evaluation distribution from one
window to the next, so a scorer can appear to degrade simply because it
reshaped the data it is later judged on.

\subsection{Why It Could Change the Headline Conclusion}

The capacity-decay study~\cite{paper6capacity} reported that HygienePrio-full's W6 P@50
...
tail. The
mechanism the capacity-decay study named is recursive: HygienePrio-full preferentially
...
mechanism would be draining the HRS upper tail. The capacity-decay study headline
