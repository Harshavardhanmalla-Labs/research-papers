...
\textit{the online-calibration study's K=200 hazard (lag-1 calibration harms at high
capacity) is reproduced.} AutoHeal at $K = 200$ achieves the
same coverage as $K = 100$ but with the same safety-flag rate and
without the calibration step (the online-calibration study's recipe says hold weights
fixed at $K = 200$, and AutoHeal does so).

\textit{the self-trajectory study Theorem~1 (closed-loop signal exhaustion) predicts
...
\textit{the HygienePrio scorer's external-validity finding (\S X of that paper;
synthetic-vs-real
...
levels shifted as predicted by the HygienePrio scorer.
