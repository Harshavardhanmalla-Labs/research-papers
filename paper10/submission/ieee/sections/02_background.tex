...
investigation and response, Tanium's auto-remediation, Tenable
Nessus / vulnerability-management workflows, IBM SOAR, and Splunk
Phantom. The unifying property
...
patch into a fleet-wide outage; the same blast-radius reasoning
underlies adversary-behaviour models such as MITRE
ATT\&CK~\cite{strom2020attack}, which catalogue how a foothold on one
asset propagates across a fleet.

\textit{(ii) False prioritisation.} The decision of \emph{which}
pair to remediate first rests on a scoring rule; the dominant rules
are CVSS~\cite{mell2007cvss}, the exploit-likelihood EPSS
model~\cite{jacobs2021epss}, and the stakeholder-specific SSVC
categorisation~\cite{spring2021ssvc}. Empirical studies show that
severity alone is a poor predictor of real-world
exploitation~\cite{allodi2014comparing}, motivating exploit-prediction
signals built from disclosure-time
data~\cite{sabottke2015vulnerability,bullough2017predicting}.
If the scoring rule that selects
patches to apply is biased, by attacker manipulation
(the HygienePrio scorer, \S XI, adversarial robustness~\cite{paper4hygieneprio}),
distributional shift
(the HygienePrio scorer, \S X~\cite{paper4hygieneprio}), or capacity-driven signal
exhaustion (the self-trajectory study, its Theorem~1), the autonomous system will
...
The hygiene-augmentation series of this research sequence developed components that
collectively address these risks:

\begin{itemize}
\item HygieneBench~\cite{paper3hygienebench} provides the
synthetic, seeded telemetry substrate, identity state, patch
posture, vulnerability exposure, and telemetry freshness, that
underlies the EEHDA fleet fixtures used throughout the sequence.
\item HygienePrio~\cite{paper4hygieneprio} provides a
scoring rule with calibrated weights, real-distribution external
validity (the HygienePrio scorer, \S X), and
quantified adversarial robustness (the HygienePrio scorer, \S XI).
\item the online-calibration study's lag-1 online calibration gives
a deployable recipe for moderate capacity ($K \leq 100$).
\item the self-trajectory study's Theorem 1 bounds when
