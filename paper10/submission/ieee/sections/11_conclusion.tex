...
AutoHeal integrates the hygiene-augmentation series of the VulnPrio sequence into a
...
the hygiene-augmentation series are sufficient to build a self-healing framework whose
\emph{safety bounds are observable in advance} and whose
\emph{failure modes are predicted by the program's theoretical
results} (the self-trajectory study Theorem~1 predicts the K=200 exclusion;
the online-calibration study's lag-1 hazard predicts the calibration recipe choice).
...
and safety bounds; do not deploy at $K = 200$ per the self-trajectory study
