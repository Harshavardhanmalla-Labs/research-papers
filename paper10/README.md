# the AutoHeal framework, AutoHeal: A Pre-Registered Self-Healing Framework for Autonomous Vulnerability Remediation

A synthesis paper integrating the hygiene-augmentation series into a six-stage closed-loop
...
- **Learn** uses the online-calibration study lag-1 calibration (only at K ≤ 100; held fixed at K = 200 per the self-trajectory study Corollary 4).
...
Dependencies: the HygienePrio scorer's `hygieneprio` + the temporal-stability study's `paper5.window_sim` + `real_data/processed/cve_corpus_for_sampling.csv`.
