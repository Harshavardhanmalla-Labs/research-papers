# the AutoHeal framework, Pre-Registration Protocol
...
**Builds on (synthesis paper):** the HygieneBench benchmark (HygieneBench), the HygienePrio scorer (HygienePrio),
The temporal-stability study (Temporal Stability), the capacity-decay study (Capacity-Indexed Decay), the online-calibration study
(Lag-1 Online Calibration), the multi-history smoothing study (Multi-History Smoothing), the self-trajectory study
...
the hygiene-augmentation series of the VulnPrio sequence established a coherent picture of
...
results: "use HygienePrio for ranking", "expect capacity-driven
collapse at high K" (the capacity-decay study), "deploy lag-1 calibration below K=100"
(the online-calibration study), "decouple scoring from selection at high K" (the self-trajectory study), and
"the +31 pp synthetic gap shrinks to +2.4 pp under real distributions"
(the HygienePrio scorer §12).
...
- **RQ4.** Does it preserve the HygienePrio scorer's per-pair dominance of
...
### Stage 2: Score
Apply the HygienePrio scorer HygienePrio scorer with calibrated weights
$(\alpha, \beta, \gamma, \delta) = (0.7, 0.5, 0.1, 0.2)$. At each
window, apply the online-calibration study lag-1 calibration **only** if cell capacity
$K \leq 100$; otherwise hold fixed the HygienePrio scorer weights (the online-calibration study's
deployable recipe).
...
exercising the online-calibration study's recommended regime). Higher-priority pairs scheduled
first.
...
### Stage 7: Learn (the online-calibration study online calibration)
...
- Arrival rate $\lambda = 3$ (the temporal-stability study canonical)
- Windows $W = 12$ (28 days of bi-weekly cycles; 2x the temporal-stability study's window count)
...
- **Fixed-policy**: classify by the HygienePrio scorer fixed weights only; no
...
|----|-----------|---------------|
| H1 | Coverage: AutoHeal remediates $\geq 80\%$ of EPSS $> 0.5$ CVEs within 14 days (W$\leq 6$). | Supported if cell-mean coverage $\geq 0.80$ at $K = 50$ and $K = 100$. (K=200 excluded by the self-trajectory study Corollary 4.) |
...
- Multi-tenant attacker model beyond the HygienePrio scorer §13's Stackelberg
 single-shot game.
- Reinforcement-learning policy updates (the self-trajectory study future work).
