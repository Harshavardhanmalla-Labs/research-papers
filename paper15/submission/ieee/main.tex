...
feeds, yet prior fusion work reports that combining sources helps without saying how much current-vulnerability
visibility is gained, by what mechanism, or what bounds it. We close that gap with a
pre-registered, reproducible simulation of a 5{,}000-asset fleet, with hypotheses, thresholds, and evaluation
seeds locked in a dated protocol before any result was inspected. At the reference coverage overlap, fusion detects
...
This paper answers those three questions with a transparent model and a pre-registered, reproducible
evaluation. We do not claim to measure any particular agency's posture; we build an explicit model of two feeds
...
significance tests. The novel contribution is to make fusion's value \emph{predictable and bounded}:
prior multisensor-fusion work establishes that combining complementary feeds helps in
aggregate~\cite{hall1997fusion,bass2000fusion} but does not quantify the current-vulnerability gain,
expose its mechanism, or identify the limit that fusion structurally cannot cross. We show the gain
is exactly the second feed's unique fresh coverage, so an operator can predict it from two measurable
tool properties before integrating anything, and that it is capped by a hard blind-spot floor that
only new coverage, not better fusion logic, can lower. Pre-registration with locked seeds makes the
study a reproducible specimen rather than a post-hoc fit. The contribution is fourfold.
...
timeliness~\cite{paper4hygieneprio,paper1vulnprio,paper3hygienebench,paper16hygieneml}; the present paper isolates the
...
\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig1_regimes.pdf}
...
\begin{figure}[t]\centering\includegraphics[width=\columnwidth]{fig2_overlap.pdf}
...
H.~Malla, ``{HygieneBench}: A reproducible synthetic benchmark for cyber-hygiene anomaly detection,'' \url{https://github.com/Harshavardhanmalla-Labs/research-papers/tree/main/paper3}, 2026, companion paper.

\bibitem{paper16hygieneml}
H.~Malla, ``Multivariate machine learning for cyber-hygiene anomaly detection,'' \url{https://github.com/Harshavardhanmalla-Labs/research-papers/tree/main/paper16}, 2026, companion paper.
