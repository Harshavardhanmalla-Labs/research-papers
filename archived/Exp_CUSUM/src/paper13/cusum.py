"""Paper 13: CUSUM detector for adaptive recalibration.

CUSUM(k, h) on |Delta_w|:
  C_w = max(0, C_{w-1} + |Delta_w| - k)
  Fire if C_w >= h; on fire reset C_w = 0.

Pre-registered: k = 0.04, h = 0.10.

Six strategies per (cell, window): fixed, lag1, cap_aware (Paper 12
control with tau_K = {50:0.20, 100:0.05, 200:0.02}), cusum, gated, offline.
"""
...
CUSUM_K = 0.04
CUSUM_H = 0.10
TAU_CAP = {50: 0.20, 100: 0.05, 200: 0.02}
...
def run_cell(K: int) -> tuple[pd.DataFrame, list[dict]]:
    ...
    rows: list[dict] = []
    cp_log: list[dict] = []
    cusum_c = 0.0  # accumulator state across windows
    ...
    for w_idx in range(N_WINDOWS):
        ...
        if w == 1:
            ...
            # CUSUM accumulator update
            cusum_c = max(0.0, cusum_c + abs(cp_delta) - CUSUM_K)
            fire_cusum = cusum_c >= CUSUM_H
            if fire_cusum:
                cusum_c = 0.0  # reset after firing

            # cap_aware decision
            fire_cap = abs(cp_delta) >= TAU_CAP[K]

        sw_cusum = FIXED_WEIGHTS if (w >= 2 and fire_cusum) else sw_lag1
        sw_cap = FIXED_WEIGHTS if (w >= 2 and fire_cap) else sw_lag1
        sw_gated = sw_lag1 if K <= GATED_K_THRESHOLD else sw_fixed

        cp_log.append({
            "cell_K": K, "window": w, "cp_delta": round(cp_delta, 4),
            "cusum_C_after": round(cusum_c, 4),
            "fire_cusum": fire_cusum, "fire_cap": fire_cap,
        })

        for s in EVALUATION_SEEDS:
            pairs, hrs = eval_[s][w_idx]
            for tag, sw in (("fixed", sw_fixed), ("lag1", sw_lag1),
                            ("cap_aware", sw_cap), ("cusum", sw_cusum),
                            ("gated", sw_gated), ("offline", sw_offline)):
                rows.append({
                    "cell_K": K, "cell_lambda": LAMBDA,
                    "seed": s, "window": w, "strategy": tag,
                    "p_at_50": round(_score_p50(pairs, hrs, sw), 4),
                })
        print(f"  W{w}: |delta|={abs(cp_delta):.3f} C={cusum_c:.3f} fire_cusum={fire_cusum} fire_cap={fire_cap}", flush=True)
