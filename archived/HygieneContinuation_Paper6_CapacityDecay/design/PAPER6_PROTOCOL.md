# the capacity-decay study, Pre-Registration Protocol
...
**Builds on:** the HygienePrio scorer (HygienePrio scorer + EEHDA generator),
The temporal-stability study (multi-window simulator + per-window evaluation).

---

## 1. Motivation

The temporal-stability study documented that EPSS-only Precision@50 decays from 0.331 at W1 to
...
regime than K = 25 / λ = 1, and the temporal-stability study's single-cell result does not tell
them which.
...
| Seeds | 25 (105-129; identical to the temporal-stability study evaluation set) |

Total cells: 5 × 4 = **20**. Total rows: 20 × 25 × 6 × 5 = **15,000**.

All other parameters inherit the temporal-stability study's pre-registration verbatim (patch debt
...
Identical to the temporal-stability study §5.4: at each window, the top-K pairs selected by
...
```
The driver re-uses the temporal-stability study's `paper5.window_sim` and the HygienePrio scorer's
`hygieneprio` package via `sys.path` injection (no code copy). Determinism
is inherited from the temporal-stability study's per-(seed, window) RNG seeding.

## 9. Out of scope

- Real fleet data: not used. All claims are bounded to the synthetic
 EEHDA evaluation context.
- Online weight relearning: studied separately (the temporal-stability study §H3 ablation;
 could be future work for the online-calibration study).
