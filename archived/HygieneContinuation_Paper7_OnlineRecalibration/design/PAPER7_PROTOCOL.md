# the online-calibration study, Pre-Registration Protocol
...
**Builds on:** the HygienePrio scorer (HygienePrio scorer + EEHDA generator), the temporal-stability study
(multi-window simulator + offline recalibration ablation), the capacity-decay study
...
the temporal-stability study's H3 ablation showed that grid-searching scorer weights at each
...
$+21.3$ pp at W2 over the HygienePrio scorer's fixed weights. That ablation uses
...
window-$w$ data. the temporal-stability study's H3 result is therefore an offline ceiling, not
a deployable procedure.

The capacity-decay study documented a parallel finding: at high capacity ($K \in \{100, 200\}$),
...
- **RQ1.** Does rolling-history online calibration beat the fixed the HygienePrio scorer
 weights at every window?
- **RQ2.** Does rolling-history online calibration approach the offline-peek
 ceiling (the temporal-stability study H3)?
- **RQ3.** Does online calibration rescue HygienePrio-full at the
 high-capacity cells where the capacity-decay study reported collapse?
...
|-----|----------|----------------------------------------|
| `fixed` | the HygienePrio scorer weights, never refit | none |
| `online` | Rolling-history grid search | calibration seeds at window $w-1$ only |
| `offline` | the temporal-stability study H3 peek calibration | calibration seeds at window $w$ |
...
The grid is identical to the temporal-stability study §H3:
...
Three capacity levels at the capacity-decay study-canonical arrival rate:
...
The high-capacity cells ($K = 100, 200$) are exactly the capacity-decay study's H4-rejected
collapse cells; the low-capacity cell ($K = 50$) anchors against the temporal-stability study's
canonical setting.
...
```
The runner reuses the temporal-stability study's window simulator and the HygienePrio scorer's scorer
