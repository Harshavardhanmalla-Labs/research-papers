# the VulnPrio study, Next Actions
...
## C. For the CalibScore study reuse

- Reuse the existing infrastructure without disturbing the VulnPrio study's frozen result:
...
- Start the CalibScore study in its own results namespace (e.g. a new `results/` subtree and
 config) so the VulnPrio study's `results/primary_full_v1/` freeze remains untouched.
- Candidate the CalibScore study directions already implied by Future Work: calibrated-weights
